...
ob_start();
session_start();
if(!isset($_SESSION['ac_no']) || !isset($_SESSION['tid']))
...
<table width="100%" height="70%">
 <tr>
  ...
  <th width="73%" style="text-align:left;vertical-align:top">
     ...
     <div class="details" style="margin-top:50px;height:235px;width:805px;background:#E2A547;opacity:1"><br><font style="color:black">
     <img style="margin-left:40px;padding-right:50px" src="success.png" height="100" width="130" align="left">
     Funds have been transfered from your account on 
     <?php
       $con=new mysqli("localhost","root","","sbi");
       $q1="SELECT * FROM tran_".$id."_t WHERE id='".$_SESSION['sid']."'";
       $q2="SELECT * FROM tran_".$_SESSION['trg']."_t WHERE id='".$_SESSION['tid']."'";
       $res1=mysqli_fetch_assoc(mysqli_query($con,$q1));
       $res2=mysqli_fetch_assoc(mysqli_query($con,$q2));
       echo $res1['date'].".";
     ?>
     <br><br>
     Please note this transaction ID for futuer reference :<?php echo $res1['id']; ?><br>
     Transaction Status Completed Successfully<br><br>
     <b>Details of Transaction</b><br><br>
     <table align="right" style="text-align:center;margin-right:70px;border-collapse:collapse">
       <tr>
         <th></th>
       ...
       <tr>
         <td style="border-top:1px solid #DDD;height:28px"><b>Debit</b></td>
         <td style="border-top:1px solid #DDD"><?php echo $id ?></td>
         <td style="border-top:1px solid #DDD">Savings Account</td>
         <td style="border-top:1px solid #DDD"><?php echo $_SESSION['bank'] ?></td>
         <td style="border-top:1px solid #DDD"><?php echo $res1['debit'] ?></td>
       </tr>
       <tr>
         <td style="border-top:1px solid #DDD;height:28px"><b>Credit</b></td>
         <td style="border-top:1px solid #DDD"><?php echo $_SESSION['trg'] ?></td>
         <td style="border-top:1px solid #DDD">Savings Account</td>
         <td style="border-top:1px solid #DDD"><?php echo $_SESSION['bank'] ?></td>
         <td style="border-top:1px solid #DDD"><?php echo $res2['credit'] ?></td>
       </tr>
     </table>
     </div>
   </div></div>
