...
<table width="100%" height="70%">
 <tr>
  <th width="34%" style="margin-left:0px;padding-top:50px">
   ...
   <form method="post">
     <font style="padding-left:0px;padding-left:30px;padding-right:10px">Customer ID*</font>
     <input type="text" name="CID" placeholder="Customer Id" style="outline:none;width:172;height:35;border:0px;background-color:#2b4769;color:white;padding-left:20px"><br><br>
     <font style="padding-left:0px;padding-left:30px;padding-right:48px">Agency*</font>
     <select name="agency" style="outline:none;width:172;height:35;border:0px;background-color:#2b4769;color:white;padding-left:20px">
         <option value="" selectsed="selected">--Select Agency--</option>
         <option value="hp">HP</option>
         <option value="ind">Indane</option>
         <option value="bhr">Bharat</option>
     </select><br>
     <?php
       if(isset($_POST['pay']))
          { if(empty($_POST['CID']) || empty($_POST['agency']))  
              ...
              { if(is_numeric($_POST['CID']))
                 { $id=$_SESSION['ac_no'];
                   ...
                   $bal=$result['balance']-637.00;
                   mysqli_query($conn,"UPDATE cb_dtb SET balance='".$bal."' WHERE account_no='".$id."'");
                   mysqli_query($con,"INSERT INTO tran_".$id."_t (id,type,debit,credit,balance) VALUES('".$id1=gen()."','gas bill payment','637',null,'".$bal."')");
     ...
     ?>
     <br>
     <button class="button1" type="submit" name="pay" style="margin-left:30px;background:blue;border:none;height:35px;width:110;color:white">Pay</button>
  ...
  <img src="LPG.jpg" height="430" width="850">
  </th>
 </tr>
</table>
